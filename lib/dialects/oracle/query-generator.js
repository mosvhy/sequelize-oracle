    ...
    insertQuery: function(table, valueHash, modelAttributes, options) {
      ...
      for (key in valueHash) {
        if (valueHash.hasOwnProperty(key)) {
          ...
          if (modelAttributeMap && modelAttributeMap[key] && !value) {
            if (modelAttributeMap[key].autoIncrement === true && !this._dialect.supports.autoIncrement.defaultValue) {
              fields.splice(-1, 1);
            ...
            if (modelAttributeMap[key].type instanceof DataTypes.NUMBER) {
              bindType = this.sequelize.connectionManager.lib.NUMBER;
            }else if (modelAttributeMap[key].type instanceof DataTypes.STRING) {
              bindType = this.sequelize.connectionManager.lib.STRING;
            }else{
              bindType = false;
            }

            if (this._dialect.supports.returnValues && options.returning && bindType) {
              if (!!this._dialect.supports.returnValues.returning) {
                valueQuery += ' RETURNING ' + this.quoteIdentifier(key) + ' INTO :rid';
                emptyQuery += ' RETURNING ' + this.quoteIdentifier(key) + ' INTO :rid';
                bindParameters = {
                  rid: { type: bindType, dir: this.sequelize.connectionManager.lib.BIND_OUT }
                };
